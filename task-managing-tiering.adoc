...
+
image:screenshot_tiering_health.gif["显示集群的分层运行状况失败的屏幕截图。"]

. 将鼠标悬停在上 image:screenshot_info_icon.gif["显示失败原因的 I 图标的屏幕截图"] 图标以查看失败原因。
. 更正问题描述：
+
...
您可以从分层_Cluster Dashboard_将未发现的内部ONTAP 集群添加到BlueXP中、以便为集群启用分层。
