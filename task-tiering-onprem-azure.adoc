...
* https://docs.netapp.com/us-en/cloud-manager-setup-admin/task-quick-start-connector-azure.html["在Azure中部署连接器"^]
...
+
** 通过端口443与BlueXP分层服务和Azure Blob对象存储建立HTTPS连接 (https://docs.netapp.com/us-en/cloud-manager-setup-admin/task-set-up-networking-azure.html#endpoints-contacted-for-day-to-day-operations["请参见端点列表"^]）
...
NOTE: 如果您计划将BlueXP分层配置为使用成本较低的访问层、以便分层数据在一定天数后过渡到、则在Azure帐户中设置容器时、不能选择任何生命周期规则。BlueXP分层可管理生命周期过渡。
...
.. *访问层生命周期*：BlueXP分层可管理分层数据的生命周期过渡。数据以 _hot_ 类开头，但您可以创建一个规则，以便在一定天数后将数据移动到 _cool_ 类。
+
