...
summary: 通过将非活动数据分层到StorageGRID来释放本地ONTAP集群上的空间。 
---
...
[role="lead"]
通过将非活动数据分层到StorageGRID来释放本地ONTAP集群上的空间。
...
* 运行ONTAP 9.4 或更高版本的本地ONTAP集群，以及通过用户指定端口到StorageGRID 的连接。  https://docs.netapp.com/us-en/bluexp-ontap-onprem/task-discovering-ontap.html["了解如何发现集群"^] 。
...
[role="quick-margin-para"]
选择本地工作环境，为分层服务选择“启用”，然后按照提示将数据分层到StorageGRID。
...
许可:: 如果您将数据分层到StorageGRID，则不需要分层许可证或FabricPool许可证。
集群网络连接要求::
+
--
...
+
设置数据分层时，系统会提示您选择要使用的 IP 空间。您应选择与每个 LIF 关联的 IP 空间。这可能是您创建的 " 默认 "IP 空间或自定义 IP 空间。详细了解 https://docs.netapp.com/us-en/ontap/networking/create_a_lif.html["LIF"^]和 https://docs.netapp.com/us-en/ontap/networking/standard_properties_of_ipspaces.html["IP 空间"^] 。
...
您需要先在 Canvas 中创建一个本地ONTAP工作环境，然后才能开始分层冷数据。
...
+
image:screenshot_setup_tiering_onprem.png["屏幕截图显示了选择本地ONTAP工作环境后出现在屏幕右侧的“设置分层”选项。"]
...
.. *Bucket*：添加一个新存储桶或选择一个以前缀 _fabric-pool_ 开头的现有存储桶，然后选择 *Continue*。
+
